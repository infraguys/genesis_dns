...
from genesis_dns.common.api.middlewares import errors as errors_mw
from genesis_dns.user_api.api import routes as app_routes
from genesis_dns.user_api.api import versions
from genesis_dns import version as app_version
...
def get_openapi_engine():
    openapi_engine = openapi_engines.OpenApiEngine(
        info=openapi_structures.OpenApiInfo(
            title=f"Genesis DNS {versions.API_VERSION_1_0} User API",
            version=app_version.version_info.release_string(),
            description=(f"OpenAPI - Genesis DNS {versions.API_VERSION_1_0}"),
        ),
